...
**控制面——协调器**（DaemonSet 部署）：主动探测 SMG endpoints；聚合探测结果为健康视图；在需要 failover 时运行确定性选举算法，对 Consul 做 CAS 乐观写；维持到 Consul 的阻塞查询订阅；通过提供 HTTP 长轮询向数据面推送 locked endpoint 变更。
...
协调器通过 K8s Service 暴露，WASM 通过 Higress 的 DNS cluster 访问 Service FQDN。这种模式下控制面和数据面的边界更清晰，协调器可以使用标准 Go 生态的任意协议库。代价是多了一个需要独立运维的组件。
...
- **控制面解耦方向 V5.3**：尝试把控制面逻辑从 WASM 拆到独立协调器，用 DaemonSet 部署，不修改 Higress。WASM 只负责订阅状态变更和请求路径重试。
