...
这里记录了我们（我和 Kimi2.6）在给 sgl-model-gateway（SMG）做高可用设计时，从 V0 到 V8 逐步探索、不断调整思路的过程。每个目录对应一个阶段的思考，下面简述每个方案提出时的动机，以及方案演进中的权衡和取舍。
...
V5.2 的解决思路是**确定性共识**：所有 Pod 通过发布订阅同步健康探测结果，每个 Pod 本地维护相同的健康视图，运行相同的确定性选择算法，独立得出相同的路由决策。不需要分布式锁。

为解決高优先级节点恢复后流量自动回切导致的 cache tree 反复迁移问题，V5.2 在后端增加了一个**全局 `locked` 状态**（`smg/locked/{group}`）——当前活跃的 endpoint 被写入共享存储，所有 Pod 通过 Pub/Sub 同步该状态。只要 locked endpoint 仍健康，即使更高优先级的节点恢复，流量也不会回切。Failover 采用**顺序降级策略**（当前 locked 失败后找下一个，而非跳回最高优先级），实现跨 Pod 的粘滞路由。

请求路径零外部调用（纯本地缓存），被动重试作为**有界分叉的本地 fail-fast** 兜底探测间隙内的故障（毫秒级），主动探测 + Pub/Sub 保证后续请求的一致性（秒级）。后端可插拔：
...
```
    ...
    │ └─────┘ │       │ └─────┘ │       │ └─────┘ │
    └────┬────┘       └────┬────┘       └────┬────┘
         │  Pub/Sub 状态同步 (Consul/Nacos/Redis)
         └─────────────────┼─────────────────┘
                           ▼
              ┌────────────┼────────────┐
              ▼            ▼            ▼
...
关于"分布式一致性动态路由决策"这个主题，我们专门写了一篇从零重新梳理的调研文章，探讨了外部协调层、分布式共识算法、一致性哈希、控制面推送、K8s 原生协调、P2P gossip、请求反馈驱动等七个方向，最终收敛到 V5.2 的结论。

→ [V5.2 调研方案](ha_design_v5.2/active_standby_ha_design_v5.2_research.draft.md)

→ [分布式一致性动态路由决策](ha_design_v5.2/distributed_consistent_dynamic_routing_research.draft.md)

## V5.3：控制面解耦与状态订阅

V5.2 的控制面逻辑（探测、聚合、CAS 选举、Consul 交互）全部实现在 WASM 插件中。WASM 的能力边界比较明确：只能发 HTTP 请求，不能持长连接做 streaming，也不能在后台跑独立的 goroutine。这意味着 etcd（需要 gRPC streaming）和 ZooKeeper（需要自定义 TCP 协议）等后端协议天然无法接入。

V5.3 的划分逻辑是**控制面与数据面分离**：控制面负责维护全局状态视图并做出路由决策，数据面负责在请求路径上执行这些决策。V5.2 中两类职责全部混杂在 WASM 内，proxy-wasm 的约束同时限制了两边。拆开后，协调器作为控制面组件不受 WASM 约束，WASM 作为数据面组件也更轻量。

**控制面——协调器**（DaemonSet 或 Sidecar 模式部署）：主动探测 SMG endpoints；聚合探测结果为健康视图；在需要 failover 时运行确定性选举算法，对 Consul 做 CAS 乐观写；维持到 Consul 的阻塞查询订阅；通过提供 HTTP 长轮询向数据面推送 locked endpoint 变更。

**数据面——WASM 插件**：维持一条到协调器的长轮询链，收到状态变更后更新本地 locked 缓存；请求路径上从本地缓存读取 locked endpoint 做路由决策；请求失败时本地标记 UNHEALTHY 并 transparently 重试到下一 endpoint。

**权威视图——Consul KV**：存储各 Pod 上报的原始探测报告和全局 locked 状态，提供 CAS 乐观并发控制和阻塞查询订阅。

协调器的暴露方式取决于部署模式：DaemonSet 通过 K8s Service 暴露，WASM 通过 Higress DNS cluster 访问；Sidecar 通过 localhost 暴露，WASM 通过 Higress Static cluster 访问。两种模式下控制面和数据面的边界都更清晰，协调器都可以使用标准 Go 生态的任意协议库。

协调器和 Gateway 需要保持网络同构：协调器探测 SMG 的路径应该和 Gateway 转发请求到 SMG 的路径一致。如果协调器离 Gateway 很远，它探测到的健康状态可能只是它自己节点的网络状况，Gateway 实际转发时未必如此。DaemonSet 或 Sidecar 把协调器和 Gateway 放在同一节点，共享同一网络环境。

代价是多了一个需要独立运维的组件。

→ [V5.3 架构设计](ha_design_v5.3/architecture_design.md)
...
## V7：镜像流量——如果所有实例都看到同样的请求呢？

V1-V5 的思路是"只让一个实例干活，树就不会分散"。V6 的思路是"让树同步起来，多个实例同时干活也没关系"。V7 尝试的是第三条路：用 Envoy 的流量镜像能力，把每个请求同时发给组内所有实例，让每个实例的树都记录完整的历史。

Envoy 的 `request_mirror_policies` 确实支持多目标镜像，镜像请求可以带自定义 header（如 `X-SMG-Shadow: true`），响应被丢弃且不阻塞原请求。如果 SMG 配合识别这个 header，只更新 tree 不转发给 PD worker，理论上每个实例的树都会包含所有请求的历史。

但深入读 `cache_aware.rs` 和 `tree.rs` 的源码后发现，这个假设不成立。SMG 的 tree 建立过程中存在至少五个随机/不确定环节：

- tree 缺失时 `rand::rng()` **显式随机**选择 worker
- load 相同时依赖 `DashMap` **不确定的迭代顺序**
- tenant 选择时 `DashMap.iter().next()` 的顺序不确定
- 时间戳更新是**概率性的**（1/8）
- node split 时的 `contains_key` + `insert` 是**非原子操作**

即使流量 100% 一致、代码逻辑相同，不同实例的 tree 结构仍然会因为这些随机性而分歧。radix tree 对插入顺序敏感，镜像流量不能保证处理时序一致。V7 被证伪。

这也反过来验证了 V6 的合理性：只有通过**有序的事件同步**才能绕过所有随机性，直接复制操作日志比试图复制流量更根本。

→ [V7 调研方案](ha_design_v7/mirror_traffic_tree_consistency_research.draft.md)

---

## V8：服务发现组件管理拓扑与健康状态

V4 用 Operator 管理 EndpointSlice，V5.1 用 WASM 插件管理 endpoint 健康。V8 尝试的是另一条路：让 SMG 实例注册到 Consul/Nacos 等服务注册中心，由注册中心负责健康检查和拓扑管理，Higress 通过 McpBridge 动态发现健康的 endpoint，Envoy 的 EDS priority 机制实现主备自动切换。

调研后发现三个核心阻碍：

- **McpBridge 不支持 priority 映射**。所有从注册中心发现的 endpoint 都被生成为 flat 的 ServiceEntry，priority 全部为 0。没有机制将注册中心 metadata（如 `role: primary`）映射为 Envoy 的 `LocalityLbEndpoints.priority`。
- **刷新延迟太高**。McpBridge 默认 30 秒、最小 10 秒的刷新间隔，远慢于 failover 所需的亚秒级感知。
- **注册中心的刷新延迟太高**。Consul/Nacos 可以配置 HTTP 健康检查来探测 SMG 的 `/readiness`，但 McpBridge 从注册中心同步状态到 Envoy 需要 10-30 秒，远慢于 failover 所需的亚秒级感知。

如果团队已经在使用 Consul/Nacos，并且愿意扩展 McpBridge 或自建 xDS 控制平面，这个方向长期有价值。但对于 immediate 需求，V4 和 V5.1 更务实。

V8 的探索也让我们意识到：无论走服务发现还是插件化路线，本质问题都是同一个——**在多实例 L7 网关环境下实现分布式一致性动态路由决策**。这个问题在 [分布式一致性动态路由决策](ha_design_v5.2/distributed_consistent_dynamic_routing_research.draft.md) 中有更系统的梳理。

→ [V8 调研方案](ha_design_v8/service_discovery_ha_research.draft.md)

---

## 阶段性总结

V0 到 V8 的演进，本质上是在回答一个问题：SMG 的有状态后端如何在多实例 L7 网关下实现一致性路由。
...
- **控制面解耦方向 V5.3**：尝试把控制面逻辑从 WASM 拆到独立协调器，DaemonSet 或 Sidecar 模式部署，不修改 Higress。WASM 只负责订阅状态变更和请求路径重试。

V6（cache tree 同步）是长期方向，如果走通则不再需要主备结构，但目前 sglang 源码中相关接口尚未完全打通。V7（镜像流量）已被证伪。V8（服务发现）因 McpBridge 限制而不适合 immediate 需求。

无论最终选择哪个实现路径，这次调研的一个重要收获是对问题本身的理解：**分布式一致性动态路由决策**——这不是简单的状态同步或负载均衡，而是在动态变化的运行时健康状态和确定性路由算法之间寻找共识。这个问题及其分析过程记录在 [分布式一致性动态路由决策](ha_design_v5.2/distributed_consistent_dynamic_routing_research.draft.md) 中。
...
- V5.2 — Pub/Sub 拓扑共识 → [V5.2 调研方案](ha_design_v5.2/active_standby_ha_design_v5.2_research.draft.md)
- V5.2 — 分布式一致性动态路由决策 → [V5.2 动态路由决策](ha_design_v5.2/distributed_consistent_dynamic_routing_research.draft.md)
- V5.2 — Consul 部署配置 → [V5.2 Consul 部署配置](ha_design_v5.2/consul_deployment.yaml)
- V5.3 — 控制面解耦与状态订阅 → [V5.3 架构设计](ha_design_v5.3/architecture_design.md)
- V6 — cache tree 同步 → [V6 调研方案](ha_design_v6/active_standby_ha_design_v6_research.draft.md)
- V7 — 镜像流量证伪 → [V7 调研方案](ha_design_v7/mirror_traffic_tree_consistency_research.draft.md)
- V8 — 服务发现组件 → [V8 调研方案](ha_design_v8/service_discovery_ha_research.draft.md)
