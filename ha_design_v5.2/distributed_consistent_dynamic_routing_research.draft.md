...
- **Redis Streams**：HTTP 短轮询（每 3s），传播延迟 0-3s。需要额外部署 HTTP 代理（Go bridge）。
...
10. **Redis Streams**
    ...
    - 用途：设计 V5.2 备选后端（Redis + Go bridge）的数据模型和订阅机制。Redis Streams 的持久化和 trim 特性适合作为探测报告的"短期日志存储"。
...
20. **Webdis — Redis HTTP 代理**
    - 来源：https://github.com/nicolasff/webdis
    - 内容：将 Redis 命令暴露为 HTTP REST API 的轻量级代理，C 语言编写，单二进制文件 <500KB
    - 用途：评估 Redis 在 WASM 环境下的 HTTP 访问方案。WASM 插件无法直接建立 Redis RESP TCP 连接，需要通过 HTTP 代理层。
