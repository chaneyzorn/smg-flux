...
| 环境已有 Consul | 直接使用，零额外组件 | — |
| 无 Consul 环境 | 部署 3 节点集群 | 部署 Redis（2 Pod） |
...
Redis 由于 Higress 的自定义 Redis host 支持，不需要 bridge，部署组件数量（2 Pod）比 Consul（3 节点）还少。但 Redis 的实时性不如 Consul（短轮询 0-3s vs blocking query 变化立即推送），且绑定 Higress 自定义扩展（可移植性差）。如果团队已有 Redis 且对实时性要求不极端，Redis 是一个务实的备选。
