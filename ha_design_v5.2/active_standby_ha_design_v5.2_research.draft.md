...
Higress Gateway 默认 `replicas=2`，每个 Pod 是独立的 Envoy 进程 + WASM VM。`proxy-wasm` 的 `SharedData` 是**进程级别**的，不跨 Pod 共享。
...
下一个请求 Group-1
    |
K8s Service 轮询到 Pod-B
    |
Pod-B (WASM VM-B) ----锁定----> SMG-1.2  (cache tree B)
```

**结果**：同一个 Group 的 cache tree 被分散到多个 worker，违背了 V5.1 "稳定锁定"的设计目标。
...
如果**所有 Pod 维护相同的实时健康视图**，并且**使用相同的确定性选择算法**，它们会独立得出**相同的锁定决策**——无需分布式锁。

```
相同的输入（同步的健康视图） + 相同的算法 = 相同的输出（锁定的 endpoint）
```

### 2.2 发布订阅的作用

Pub/Sub 只负责传播**健康探测结果**，不负责协调**锁定决策**。锁定是每个 Pod 本地独立计算的。
...
| 状态共享机制 | 内存共享 | 发布订阅 |
| 锁定决策 | 本地缓存 + SharedData | 本地缓存（由 pub/sub 同步） |
| 跨 Pod 一致性 | 无 | 确定性共识 |
| 跨生命周期 | 丢失 | 后端持久化 |
| 分布式锁 | 不需要（单进程内 lease） | **不需要**（确定性算法） |
...
**Key 结构**：
```
smg/health/{group}/{endpoint}/{source}
```
...
**Value**：
```json
{"healthy": true, "ts": 1234567890000}
```

**操作**：
- Bootstrap：`GET /v1/kv/smg/health?recurse=true`
- 订阅：`GET /v1/kv/smg/health?recurse=true&index={lastIndex}&wait=30s`
- 发布：`PUT /v1/kv/smg/health/{group}/{endpoint}/{source}`

每个 endpoint 的每个 source 独立为一个 key。订阅时使用 `?recurse=true` 监听整个前缀，任何 key 的变化都会触发 blocking query 返回。blocking query 在 proxy-wasm 的异步模型下完美工作：最长等待 30 秒，响应必定在 0-30 秒内完成，触发 `OnHttpCallResponse` 回调。

### 3.3 Redis 后端数据模型（备选）

如果环境已有 Redis 但没有 Consul，Redis + Go bridge 作为备选方案：
...
type TopologyBackend interface {
    ...
    // BootstrapFullState 启动时拉取全量健康状态
    BootstrapFullState() (map[string]map[string][]ProbeReport, error)
...
| **Redis** | RESP (TCP) | 需 HTTP 代理层 | **需要** Go bridge | 近实时 | 备选 |
...
```
GET /v1/kv/smg/health?recurse=true&index={lastIndex}&wait=30s
```
...
- 订阅时使用 `?recurse=true` 监听整个 `smg/health/` 前缀
- Bootstrap 时使用 `GET /v1/kv/smg/health?recurse=true`（不加 index，立即返回）
...
每个 endpoint 的每个 source 独立为一个 key。订阅时使用 `?recurse=true` 监听整个 `smg/health/` 前缀，任何变化都会触发 blocking query 返回。发布时使用 `PUT /v1/kv/<key>` 写入本 Pod 的探测结果。
...
- **部署复杂度**：如果环境没有 Consul，需要部署 3 节点 Consul server 集群。但生产环境的 Redis 同样不能单实例运行（否则是单点故障），至少需要 1 master + 1 replica + bridge（2 副本），且 bridge 是自定义代码。两者在生产环境的部署复杂度相当，Consul 是成熟软件（官方 Helm chart），Redis + bridge 需要维护自定义代理代码。
- **写入吞吐量**：Consul KV 的写入性能低于 Redis（基于 Raft 共识的持久化）。但 health check 场景写入频率很低（每个 Pod 每秒几次），不构成瓶颈。
- **并发限制**：Consul 默认限制 512 个并发 blocking query。Gateway Pod 数量为 2-10 个，每个 Pod 发起 1 个 blocking query（监听前缀），远低于限制。

**（5）Consul vs Redis + bridge**

| 维度 | Consul | Redis + Go bridge |
|------|--------|-------------------|
| 组件数量 | 1（Consul 集群，3 节点） | 2（Redis + bridge） |
| HTTP 代理 | 不需要 | 需要 |
| 运维熟悉度 | 取决于团队 | Redis 更普遍 |
| 部署复杂度 | 中（3 节点集群，官方 Helm） | 中（Redis HA + 自定义 bridge，需维护代理代码） |
| Pub/Sub 机制 | HTTP blocking query（长轮询） | HTTP 短轮询（每 3s） |
| 同步延迟 | 0-30s（变化立即推送） | 0-3s（轮询间隔） |
| 环境已有 Consul | 直接使用，零额外组件 | — |
| 无 Consul 环境 | 部署 3 节点集群 | 部署 Redis + bridge |

**结论**：Consul KV 是 V5.2 的默认推荐后端。原生 HTTP 接口与 proxy-wasm 的异步模型完美适配，blocking query 提供了比 Redis 短轮询更优的实时性和效率，且不需要任何代理层。如果团队环境已有 Consul，直接使用即可；如果还没有 Consul，部署一个 3 节点 Consul server 集群的复杂度与部署 Redis + Go bridge 相当，但减少了组件数量和自定义代码的维护负担。
...
type ConsulBackend struct {
    ...
    updateCh     chan<- HealthUpdate // 订阅通道
...
func (c *ConsulBackend) SubscribeHealthUpdates(updateCh chan<- HealthUpdate) error {
    c.updateCh = updateCh
    // Bootstrap 和订阅由外部 RegisterTickFunc 驱动
    return nil
}

func (c *ConsulBackend) BootstrapFullState() (map[string]map[string][]ProbeReport, error) {
    // 异步 Bootstrap：发起 HTTP GET，在 callback 中推送结果到 updateCh
    c.httpClient.Get(
        "/v1/kv/smg/health?recurse=true",
        nil,
        func(statusCode int, headers http.Header, body []byte) {
            if statusCode != 200 {
            ...
            c.bootstrapped = true
            // Bootstrap 完成后立即启动 blocking query 订阅
            c.startBlockingQuery()
        },
    )
    
    return make(map[string]map[string][]ProbeReport), nil
}
```
...
func (c *ConsulBackend) doBlockingQuery() {
    url := fmt.Sprintf("/v1/kv/smg/health?recurse=true&index=%d&wait=30s", c.lastIndex)
...
func (c *ConsulBackend) parseAndPush(kv ConsulKVEntry) {
    ...
    // Consul KV API 返回的 Value 是 base64 编码的字符串
...
**说明**：blocking query 在 proxy-wasm 中通过 `OnHttpCallResponse` 回调递归实现。每次响应完成后立即发起下一个请求，形成"请求 → 等待 → 回调 → 再请求"的循环。不需要 goroutine，不需要持久连接。单个 Gateway Pod 只维持 1 个 pending 的 blocking query，Consul 默认支持 512 个并发 blocking query，完全够用。
...
func (c *LocalHealthCache) UpdateReport(group, endpoint string, report ProbeReport) {
    ...
    // 追加报告，过滤过期
    c.reports[group][endpoint] = append(c.reports[group][endpoint], report)
...
func selectLockedEndpoint(group GroupConfig, cache *LocalHealthCache) string {
    // group.Endpoints 按优先级排序（配置中定义的顺序）
    for _, ep := range group.Endpoints {
        status := cache.GetHealthStatus(group.Name, ep.Name)
        if status == HEALTHY {
            return ep.Name
        }
    }

    // 全部不健康：仍锁定到最高优先级的 endpoint（fail-open，允许请求到达后触发 retry）
    return group.Endpoints[0].Name
}
```

### 8.2 为什么不需要分布式锁

假设 Group-1 有 3 个 endpoint：`[SMG-1.1, SMG-1.2, SMG-1.3]`（按优先级排序）。

**场景 A：全部健康**
- 所有 Pod 的健康视图：`{1.1: HEALTHY, 1.2: HEALTHY, 1.3: HEALTHY}`
- 所有 Pod 运行 `selectLockedEndpoint` -> 都选择 `SMG-1.1`
- ✅ 一致

**场景 B：SMG-1.1 不健康**
- Pod-A 探测到 1.1 故障，发布更新
- 所有 Pod 收到更新，健康视图：`{1.1: UNHEALTHY, 1.2: HEALTHY, 1.3: HEALTHY}`
- 所有 Pod 运行算法 -> 都选择 `SMG-1.2`
- ✅ 一致

**场景 C：网络分区（Pod-A 与 SMG-1.1 网络不通，但 Pod-B 可以连通）**
- Pod-A 视图：`{1.1: UNHEALTHY, ...}`
- Pod-B 视图：`{1.1: HEALTHY, ...}`
- 此时两 Pod 会做出不同决策 ❌

**但**：Fail-fast 聚合策略下，Pod-A 的 UNHEALTHY 报告会被发布到后端。如果 Pod-B 的订阅正常工作，它会在几秒内收到 Pod-A 的报告。此时 Pod-B 的聚合结果变为 UNHEALTHY（因为任一报告不健康）。

如果 Pod-B 的订阅也断开了（与后端网络分区），那它会使用本地缓存。缓存有过期时间，过期后如果没有新数据，状态变为 UNKNOWN。此时 Pod-B 的决策可能与其他 Pod 不一致，直到网络恢复。

这是**最终一致性**的权衡，在分布式系统中是不可避免的。
...
**关键点**：
- **零外部调用**：没有 Redis/Consul/Nacos 查询
- **纯本地内存**：O(n) 复杂度，n = 每个 Group 的 endpoint 数量（通常 2-5）
- **延迟**：与 V5.1 相同（<1μs）
...
重试的同时，通过 `RegisterTickFunc` 加速下一次探测（WASM 中不能使用 goroutine 做后台任务）：

```go
func onRequestFailure(group, endpoint string) {
    // 1. 更新本地缓存，标记该 endpoint 为疑似不健康
    ...
    // 2. 下一次 tick（最多 3s 后）会自然发布探测结果到后端
    // 不需要显式触发后台 goroutine
}
```
...
| 状态同步 | 发布到后端，影响所有 Pod | 本地执行，同时触发即时探测加速同步 |
...
Pod-D 启动（ Envoy + WASM VM 初始化）
  ...
  |     -> 此时已有完整的健康视图
  |
  |-- 4. SubscribeHealthUpdates()
  |     -> 通过 RegisterTickFunc 周期性轮询增量更新
  |
  |-- 5. 开始接收请求
  |     -> 第一个请求到达时，healthCache 已有数据
  |     -> selectLockedEndpoint 立即做出正确决策
  |
  |-- 6. startHealthCheck()
  |     -> 开始独立探测，发布结果
```
...
| **时钟漂移** | Pod 间时钟差几秒。时间窗口过滤（10s）可容忍小幅漂移 | 小幅漂移不影响，大幅漂移可能导致聚合异常 |
...
| **代码复杂度** | 中等（lease、CAS、缓存失效） | 中等（backend 抽象、聚合逻辑） |
| **部署复杂度** | 低（纯插件） | 中（需部署后端） |
...
- 实现 `ConsulBackend`（原生 HTTP blocking query）
- 实现 `LocalHealthCache` + 确定性锁定算法
- 实现健康检查 + 增量发布
...
### Phase 3: Redis 实现（备选）
- 实现 `RedisBackend`（通过 Go bridge HTTP 代理）
- 适合已有 Redis 但没有 Consul 的环境
...
12. **Envoy request_mirror_policies**
